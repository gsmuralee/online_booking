...
describe Booking do
   before(:each) { 
    ...
    @table_1 = FactoryGirl.create :table
   }
  ...
  it "#prev booking should not end during current booking" do
    build(:booking, start_time: DateTime.now + (15.minutes), duration: 15, user_id: @user.id, table_id: @table_1.id).should_not be_valid
  end

  it "#prev booking should not start during current booking" do
    build(:booking, start_time: DateTime.now + (25.minutes), duration: 40, user_id: @user.id, table_id: @table_1.id).should_not be_valid
  end
  
  it "#prev booking should not happen during current booking" do
    build(:booking, start_time: DateTime.now + (25.minutes), duration: 10, user_id: @user.id, table_id: @table_1.id).should_not be_valid
  end

  it "#prev booking should not envelop current booking" do
    build(:booking, start_time: DateTime.now + (10.minutes), duration: 50, user_id: @user.id, table_id: @table_1.id).should_not be_valid
  end

  it "#prev booking should not be identical to current booking" do
    build(:booking, start_time: DateTime.now + (15.minutes), duration: 30, user_id: @user.id, table_id: @table_1.id).should_not be_valid
  end
